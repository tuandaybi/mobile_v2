    ...
    <script>
        ...
        async function loadReleases(showError = true) {
            if (!getToken()) {
                releases = [];
                updateCounts();
                if (showError) showToast('Nhap ma Bearer de tai du lieu', 'error');
                return false;
            }
            const response = await fetch(listUrl, { headers: { Accept: 'application/json', Authorization: 'Bearer ' + getToken() } });
            const payload = await parseResponse(response);
            if (!response.ok) {
                if (showError) showToast('Khong tai duoc du lieu. HTTP ' + response.status, 'error');
                return false;
            }
        ...
        async function loadTrash(showError = false) {
            ...
            const payload = await parseResponse(response);
            if (!response.ok) {
                if (showError) showToast('Khong tai duoc thung rac. HTTP ' + response.status, 'error');
                return false;
            }
        ...
        async function moveToTrash(deleteUrl, label) {
            if (!getToken()) { showToast('Can co token Bearer de xoa', 'error'); return; }
            if (!confirm('Dua ban phat hanh ' + label + ' vao thung rac?')) return;
            const response = await fetch(deleteUrl, { method: 'DELETE', headers: { Accept: 'application/json', Authorization: 'Bearer ' + getToken() } });
            const payload = await parseResponse(response);
            if (!response.ok) { showToast((payload && payload.message) || 'Xoa that bai', 'error'); return; }
            await loadData('trash');
            showToast((payload && payload.message) || 'Da chuyen vao thung rac');
        }
        async function restoreFromTrash(restoreUrl) {
            if (!getToken()) { showToast('Can co token Bearer de khoi phuc', 'error'); return; }
            const response = await fetch(restoreUrl, { method: 'POST', headers: { Accept: 'application/json', Authorization: 'Bearer ' + getToken() } });
            const payload = await parseResponse(response);
            if (!response.ok) { showToast((payload && payload.message) || 'Khoi phuc that bai', 'error'); return; }
            await loadData('trash');
            showToast((payload && payload.message) || 'Da khoi phuc');
        }
        function copyLink(link) { navigator.clipboard.writeText(link); showToast('Da sao chep lien ket'); }

        const uploadEnabled = true;
        async function handleUpload(event) {
            event.preventDefault();
            if (!uploadEnabled) { showToast('Chua cho phep upload', 'error'); return; }
            ...
            try {
                ...
                toggleModal(false);
                showToast(payload.message || 'Upload thanh cong');
            } catch (error) {
                showOutput(getEl('uploadOutput'), String(error));
                showToast('Khong gui duoc yeu cau upload', 'error');
            } finally {
        ...
        if (refreshBtn) refreshBtn.addEventListener('click', () => loadData().catch(() => showToast('Lam moi that bai', 'error')));
        hydrateState();
